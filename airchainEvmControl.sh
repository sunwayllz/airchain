...
function install_dependencies() {
    ...
    if command -v go >/dev/null 2>&1; then
        ...
        echo "下载并安装 Go..."
        wget -c https://golang.org/dl/go1.22.4.linux-amd64.tar.gz -O - | tar -xz -C /usr/local
...
EOF
    ...
    cd $HOME
    wget https://github.com/airchains-network/tracks/releases/download/v0.0.3/eigenlayer
    chmod +x eigenlayer
...
function restartSendAccount_python() {
    ...
    # 下载python脚本
    if [ ! -e $py_file ]; then
        wget -O $py_file https://raw.githubusercontent.com/sunwayllz/airchain/main/airchainSendAccount.py
        pip install web3
...
function restartMonitor_shell() {
    ...
    if [ ! -e $shell_file ]; then
        wget -O $shell_file https://raw.githubusercontent.com/sunwayllz/airchain/main/airchainMonitor.sh
        chmod +x $shell_file
    fi
    # 运行定时重启回滚脚本
